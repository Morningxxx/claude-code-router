...
export async function checkForUpdates(currentVersion: string) {
  try {
    ...
    const { stdout } = await execPromise("npm view @morningxxx/claude-code-router version");
...
export async function performUpdate() {
  try {
    ...
    const { stdout, stderr } = await execPromise("npm update -g @morningxxx/claude-code-router");
